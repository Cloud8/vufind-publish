...
class DataStorage {
    ...
    public function __construct($db, $domain, $params = []) {
        $this->db = $db;
        $this->domain = $domain;
    ...
    public function read($identifier) {
        ...
        $did = substr($identifier,0,5) == 'dpub:' ? substr($identifier,5):false;
    ...
    public function delete($oid) {
        ...
        if ($res == 2) {
            ...
            $sql = 'DELETE p, f from opus_publications p, opus_files f'
                 .' where p.oid=f.oid and p.oid='.$oid;
    ...
    private function createURN($uid) {
        ...
        $pos = strpos($urn, '/');
        $urn = $pos>0 ? substr($urn, 0, $pos).substr($urn,$pos+1) : $urn;
        $urn = str_replace('/','-',$urn);
        $tst = $this->urn_prefix . $urn;
        $code = '3947450102030405060708094117############181419151621222324'
              . '2542262713282931123233113435363738########43';
        $chk = null;
        for ($i=$sum=0, $pos=1; $i<strlen($tst); $i++) {
            list($v1,$v2) = [
                substr($code,(ord(strtoupper(substr($tst,$i,1)))-45)*2,1),
                substr($code,(ord(strtoupper(substr($tst,$i,1)))-45)*2+1,1)
            ];
