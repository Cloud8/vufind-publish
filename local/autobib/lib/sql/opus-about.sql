...
select distinct o.title, o.creator_corporate, o.subject_swd, o.description,
        ...
        o.type as typeid, r.type, r.dini_publtype, 
        o.source_opus, o.source_title,
        ...
        o.lic, lc.link as license, o.status, o.bem_extern, o.bereich_id,
        from_unixtime(d.date_accepted,'%Y-%m-%d') date_accepted, d.advisor,
        i.inst_nr, inst.name inst_name, inst.faculty, f.name faculty_name,
        s.sachgruppe, s.sachgruppe_en, dom.ip as accessRights, 
        o.language, l.sprache as lang, l.iso_639_1,
        l.code as iso_639_2, l2.iso_639_1 as iso_lang2
 from opus o 
        ...
        left join faculty f on f.nr=inst.faculty
        left join sachgruppe_ddc s on o.sachgruppe_ddc=s.nr
        left join license lc on o.lic=lc.shortname
        left join opus_domain dom on o.bereich_id=dom.id and dom.ip is not null
 ...
 select c.coll_id, c.coll_name, c.url, c.urn, c.description
         from opus_coll oc, collections c
